...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    private void handleVoucherOrder(VoucherOrder voucherOrder) {
        ...
        if (!isLock) {
            //获取失败,返回错误或者重试
            throw new RuntimeException("下单时获取锁对象失败");
        }
        try {
...
//    public Result seckillVoucher(Long voucherId) { // 阻塞队列实现秒杀
//        //获取用户
...
//    }
    ...
    @Override
    public Result seckillVoucher(Long voucherId) { // redis消息队列实现秒杀
        //获取用户
        UserDTO user = UserHolder.getUser();
        //生成订单id
        Long orderId = redisIdWorker.nextId("order");
...
//    @Override
//    public Result seckillVoucher(Long voucherId) { // setnx锁 / redisson锁
//        //查询优惠券
...
////        boolean isLock = lock.tryLock(1200L); //获取锁
//
//        // 使用redisson分布式锁
//        RLock lock = redissonClient.getLock("lock:order:" + userId);
    ...
    public Result getResult(Long voucherId) {
        ...
        boolean isSuccess = seckillVoucherService.update(
                new LambdaUpdateWrapper<SeckillVoucher>()
                        ...
                        .gt(SeckillVoucher::getStock, 0) // greater than
                        .setSql("stock=stock-1"));
        if (!isSuccess) {
            //库存不足
            return Result.fail("库存不足");
